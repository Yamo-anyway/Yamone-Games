...
s, n = re.subn(
    r'@Composable\ninternal fun AdFreeTimeCard\(.*?\n\}\n\n(?=@Composable\ninternal fun DevelopmentBannerAd)',
    lambda _: card,
    s,
    count=1,
    flags=re.S,
...
if n != 1:
    raise SystemExit(f'AdFreeTimeCard replacement failed: {n}')
...
if n != 1:
    raise SystemExit(f'AdAccessDetailsDialog replacement failed: {n}')

helper_anchor = '''internal fun adFreeUntilText(untilMillis: Long): String {
...
'''
if 'internal fun promotionUntilText' not in s:
    if helper_anchor not in s:
        raise SystemExit('promotionUntilText anchor not found')
