...
try:
    ...
    # The first visible switches are master, music, effects and buttons.
    ...
    for index,title,start in [(5,'스도쿠',None),(6,'눈덩이 러시','시작하기'),(7,'물고기 냠냠','일반 모드'),(8,'빙하 점프','시작하기')]:
        ...
        if start:
            ...
            shell('input','tap',str(pause_point[0]),str(pause_point[1]));time.sleep(.5)
            node('잠깐 쉬어가요');screenshot(f'{index:02d}-pause')
            ensure(True,title+' starts and pauses')
            tap('계속하기')
        else:
            time.sleep(1);node('메모');screenshot('05-sudoku');ensure(True,'Sudoku board and controls rendered')
